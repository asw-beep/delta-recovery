...
| **No idempotency keys** on Payments/Payment Links (that is a RazorpayX feature). `reference_id` *usually* rejects duplicates with an error rather than returning the original — but **not always**: two live links have been observed sharing one `reference_id` after the first was cancelled (`docs/difficulties.md` #12). | Own idempotency ledger; it is the guard that actually binds, not this one. On a duplicate-reference error, treat as success and resolve via `GET /v1/payment_links?reference_id=…`. |
...
| **`GET /v1/payments/downtimes` returns only what is down right now.** A resolved outage stops appearing rather than being returned with an `end`, and `payment.downtime.resolved` is not guaranteed to arrive. | Reconcile in both directions: close any open row the endpoint has stopped reporting. Upsert alone leaves every downtime open forever (`docs/difficulties.md` #15). |
